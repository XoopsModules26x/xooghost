...
class xooghost_page extends \XoopsObject
{
    ...
    public function __construct()
    {
        ...
        // Module
        $ghost_module = \XoopsModules\Xooghost\Helper::getInstance();
        $this->config = $ghost_module->LoadConfig();
        $this->rldHandler = $ghost_module->RldHandler();
    }
        ...
        if (!in_array($this->php_self, $this->exclude_page, true) || 'index' == $this->php_self || 'page_print' == $this->php_self) {
            if ($xoops->registry()->offsetExists('XOOTAGS') && $xoops->registry()->get('XOOTAGS')) {
                ...
                if (!isset($tags[$this->getVar('xooghost_id')])) {
                    $xootagsHandler = \XoopsModules\Xootags\Helper::getInstance()->getHandler('Tags'); //$xoops->getModuleHandler('xootags_tags', 'xootags');
...
class XooghostXooghost_pageHandler extends \XoopsPersistableObjectHandler
{
    ...
    public function SetLike_Dislike($page_id, $like_dislike)
    {
        if (0 != $page_id) {
            ...
            if (is_object($page) && 0 != count($page)) {
                ...
                if ($ret = $this->rldHandler->SetLike_Dislike($page_id, $like_dislike)) {
                    if (0 == $like_dislike) {
    ...
    public function upload_images($image_name)
    {
        ...
        $uploader = new \XoopsMediaUploader(
            $xoops->path('uploads') . '/xooghost/images',
            $autoload['mimetypes'],
